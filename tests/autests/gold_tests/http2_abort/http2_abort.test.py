...
client.Streams.stdout += Testers.ContainsExpression(
    'Submitted RST_STREAM frame for key 1 on stream 1.', 'Submitted RST_STREAM frame.')

server.Streams.stdout += Testers.ContainsExpression(
    'Received an HTTP/2 request for key 1 with stream id 1', 'Server is functional.')

server.Streams.stdout += Testers.ExcludesExpression('RST_STREAM', 'Server is not affected.')

proxy.Streams.stdout += Testers.ContainsExpression(
    'Received RST_STREAM frame with error code INTERNAL_ERROR', 'Received RST_STREAM frame.')

proxy.Streams.stdout += Testers.ContainsExpression(
    'Frame sequence from client: HEADERS, DATA, RST_STREAM', 'Frame sequence.')
...
client.Streams.stdout += Testers.ExcludesExpression('Timed out waiting for frame: HEADERS',
                                                    'Await HEADERS')
...
proxy.Streams.stdout += Testers.ContainsExpression(
    'Received RST_STREAM frame with error code STREAM_CLOSED', 'Received RST_STREAM frame.')

proxy.Streams.stdout += Testers.ContainsExpression(
    'Frame sequence from client: HEADERS, RST_STREAM', 'Frame sequence.')
...
client.Streams.stdout += Testers.ContainsExpression('Received RST_STREAM frame with stream id 1',
...
client.Streams.stdout += Testers.ContainsExpression('Submitted GOAWAY frame for key 1.',
                                                    'Submitted GOAWAY frame.')

client.Streams.stdout += Testers.ExcludesExpression('should_not_send',
                                                    'Client connection should terminate.')

server.Streams.stdout += Testers.ExcludesExpression('GOAWAY', 'Server is not affected.')

server.Streams.stdout += Testers.ExcludesExpression('should_not_send',
                                                    'Server connection should terminate.')

proxy.Streams.stdout += Testers.ContainsExpression(
    'Received GOAWAY frame with error code STREAM_CLOSED', 'Received GOAWAY frame.')

proxy.Streams.stdout += Testers.ContainsExpression('Frame sequence from client: HEADERS, GOAWAY',
                                                   'Frame sequence.')
...
client.Streams.stdout += Testers.ContainsExpression('Received GOAWAY frame with last stream id 0',
                                                    'GOAWAY pass through.')
...
server.Streams.stdout += Testers.ContainsExpression('Submitted GOAWAY frame for key 1.',
                                                    'Submitted GOAWAY frame.')
...
client = r.AddClientProcess('client6', 'replay_files/client_rst_stream_mixed_data.yaml')
server = r.AddServerProcess('server6', 'replay_files/client_rst_stream_mixed_data.yaml')

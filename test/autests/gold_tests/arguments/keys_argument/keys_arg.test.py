...
for uuid in range(1, 6):
    client.Streams.stdout += Testers.ContainsExpression(f'uuid: {uuid}', f"Client has uuid {uuid}.")

#
# Test 2: Verify a single transaction can be selected with --keys.
#
r = Test.AddTestRun("Verify all keys are sent when --keys is not used.")
client = r.AddClientProcess("client2", "five_transactions.yaml", other_args="--keys 2")
...
client.Streams.stdout += Testers.ContainsExpression('uuid: 2', "Client has uuid 2.")

for uuid in [1, 3, 4, 5, 6]:
    client.Streams.stdout += Testers.ExcludesExpression(f'uuid: {uuid}', f"Client has uuid {uuid}.")
...
client = r.AddClientProcess("client3", "five_transactions.yaml", other_args="--keys 3 5")
...
client = r.AddClientProcess("client4", "five_transactions.yaml", other_args="--keys does_not_exist")
...
client.Streams.stdout += Testers.ContainsExpression('Parsed 0 transactions',

...
client = r.AddClientProcess("client1", "replay_files/five_transactions",
                            http_ports=[8082],
                            other_args="--verbose diag")
server = r.AddServerProcess("server1", "replay_files/five_transactions",
...
#
r = Test.AddTestRun("Verify all keys are sent when --keys is not used.")
client = r.AddClientProcess("client2", "replay_files/five_transactions",
                            http_ports=[8082],
                            other_args="--keys 2 --verbose diag")
server = r.AddServerProcess("server2", "replay_files/five_transactions",
                            http_ports=[8083], other_args="--verbose diag")
...
#
r = Test.AddTestRun("Verify all keys are sent when --keys is not used.")
client = r.AddClientProcess("client3", "replay_files/five_transactions",
                            http_ports=[8082],
                            other_args="--keys 3 5 --verbose diag")
server = r.AddServerProcess("server3", "replay_files/five_transactions",
                            http_ports=[8083], other_args="--verbose diag")

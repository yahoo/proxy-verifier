...
client = r.AddClientProcess("client1", "unique_by_host.yaml", other_args="--format '{url}'")
server = r.AddServerProcess("server1", "unique_by_host.yaml", other_args="--format '{url}'")
...
client.Streams.stdout += Testers.ContainsExpression(
    "Parsed 3 transactions", "Three transactions should be parsed by the client.")
...
client.Streams.stdout += Testers.ContainsExpression(
    'Key: "/same/path"', "The key should be parsed from the URL, not the uuid.")
...
server.Streams.stdout += Testers.ContainsExpression(
    "Ready with 1 transaction", "Only one transaction should be parsed by the server.")
...
client = r.AddClientProcess("client2", "unique_by_host.yaml", other_args="--format '{field.host}'")
server = r.AddServerProcess("server2", "unique_by_host.yaml", other_args="--format '{field.host}'")
...
client.Streams.stdout += Testers.ContainsExpression(
    'Key: "host.one"', "The key should be parsed from the host, not the uuid.")

server.Streams.stdout += Testers.ExcludesExpression(
    "Violation:", "There should be no verification errors because there are none added.")

server.Streams.stdout += Testers.ContainsExpression(
    "Ready with 3 transactions", "Three transactions should be parsed by the server.")
...
client.Streams.stdout += Testers.ContainsExpression(
    'Key: "host.one//same/path"', "The key should be parsed from the host/url, not the uuid.")
...
server.Streams.stdout += Testers.ContainsExpression(
    'sending a 404', "The server should send a 404 response for an unrecognized key.")
...
client = r.AddClientProcess("client7", "unique_by_url.yaml", other_args="--format '{url}'")
server = r.AddServerProcess("server7", "unique_by_url.yaml", other_args="--format '{url}'")

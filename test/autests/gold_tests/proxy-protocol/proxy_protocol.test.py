...
class PPTest:
    ...
    def setupClient(self, isHTTPS):
        self.client = self.testRun.AddClientProcess(f"client-{PPTest.test_id}", self.replayFile,
                                                    configure_http=not isHTTPS,
                                                    configure_https=isHTTPS)

    def setupServer(self, isHTTPS):
        self.server = self.testRun.AddServerProcess(f"server-{PPTest.test_id}", self.replayFile,
                                                    configure_http=not isHTTPS,
                                                    configure_https=isHTTPS)
    ...
    def setupProxy(self, isHTTPS):
        ...
        self.proxy = self.testRun.AddProxyProcess(f"proxy-{PPTest.test_id}",
...
client.Streams.stdout += Testers.ExcludesExpression(
    "Sending PROXY header", "Client should not send PROXY header if not asked to.")
proxy.Streams.stdout += Testers.ExcludesExpression("Received .* bytes of Proxy Protocol",
                                                   "Proxy should not receive the PROXY header.")
server.Streams.stdout += Testers.ExcludesExpression(
    "Received PROXY header", "The server should not receive the PROXY header.")
...
server = r.AddServerProcess("ipv6-server1",
                            "replay_files/single_transaction_ipv6_pp_v1.replay.yaml", use_ipv6=True)
client = r.AddClientProcess("ipv6-client1",
                            "replay_files/single_transaction_ipv6_pp_v1.replay.yaml", use_ipv6=True,
                            http_ports=[server.Variables.http_port], other_args="--no-proxy")
...
client.Streams.stdout += Testers.ContainsExpression(
    rf"Sending PROXY header", "Verify that the PROXY header is sent from the client to the server.")
...
#
r = Test.AddTestRun("Verify the PROXY message can be sent and received on IPv6 connection")
server = r.AddServerProcess("ipv6-server2",
                            "replay_files/single_transaction_ipv6_pp_v2.replay.yaml", use_ipv6=True)
client = r.AddClientProcess("ipv6-client2",
                            "replay_files/single_transaction_ipv6_pp_v2.replay.yaml", use_ipv6=True,
                            http_ports=[server.Variables.http_port], other_args="--no-proxy")
...
# Add configure_https=False to verify ATS client and server work when the https
# optional arguments are not provided.
client = r.AddClientProcess("specified-addr-pp-client1",
                            "replay_files/http_pp_v1_with_address.replay.yaml",
                            configure_https=False)
server = r.AddServerProcess("specified-addr-pp-server1",
                            "replay_files/http_pp_v1_with_address.replay.yaml",
                            configure_https=False)
proxy = r.AddProxyProcess("specified-addr-pp-proxy1", listen_port=client.Variables.http_port,
                          server_port=server.Variables.http_port)
...
client.Streams.stdout += Testers.ContainsExpression(
    rf"Sending PROXY header", "Verify that the PROXY header is sent from the client to the proxy.")

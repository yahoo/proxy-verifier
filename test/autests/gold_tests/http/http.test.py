...
client.Streams.stdout = Testers.ContainsExpression("7 transactions in 5 sessions",
                                                   "Verify that 7 transactions were parsed.")

client.Streams.stdout += Testers.ContainsExpression("Loading 3 replay files.",
                                                    "Verify that 3 replay files were parsesd.")

client.Streams.stdout += Testers.ContainsExpression("204 No Content",
                                                    "Verify the No Content reason string.")

client.Streams.stdout += Testers.ContainsExpression("POST /some/request HTTP/1.0",
                                                    "Verify that we sent the HTTP/1.0 version.")

client.Streams.stdout += Testers.ExcludesExpression(
    "Violation:", "There should be no verification errors because there are none added.")

server.Streams.stdout = Testers.ContainsExpression("Ready with 7 transactions.",
                                                   "Verify that 7 transactions were parsed.")

server.Streams.stdout += Testers.ContainsExpression("Loading 3 replay files",
